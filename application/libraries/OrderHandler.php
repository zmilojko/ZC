...
class OrderHandler {
  function __construct($db_order = null) {
     if(!$db_order) {
      return;
     }
  ...
  public static function get_order_index_array($last_known_timestamp, $status_filter, $index_list)
  {
      $ci =& get_instance();
      $query_build = $ci->db->select('orders.orders_id')
                              ->from('orders')
	...
	public static function get_full_orders()
	{
    $ci =& get_instance();  
    ...
    if($indexes_only)
    {
      OrderHandler::get_order_index_array($last_known_timestamp, $status_filter, $index_list);
      return;
    }
    ...
    $query = $ci->db->query($query_text)->result();

    $orders = array();     
     
    foreach($query as $row)
    {
      $orders[] = new OrderHandler($row);
